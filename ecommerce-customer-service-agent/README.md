...
当前页面采用真实店铺客服会话形态，不放置不可操作的会话列表、测试按钮或内部处理路径。下图展示：用户完成 `DD1001` 查询后继续问“物流到哪里了”，系统复用已验证订单上下文，并通过受控物流 Tool 返回最新节点。

![新版客服会话：已验证订单上下文复用](../docs/screenshots/ecommerce-customer-chat-current.png)
